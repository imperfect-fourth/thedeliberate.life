...
Wait what? You don't believe me? Check it out yourself on the [Go playground](https://go.dev/play/p/FjcKgYH2bmx).
...
The different behaviour between **uint** and **int** is because Go does a
...
u >>= 6         // 0b00000001
```
Right-shifts "empty" the most significant bits, which are then filled in with
0. Similarly, left-shifts "empty" the least significant bits which are then
filled in with 0.
...
signedness, whereas logical shifts don't.

Before we follow along an example, first thing to note is that a right-shift is
essentially a division by 2, and a left-shift is multiplication by 2.

Second is to remember that signed integers are stored in two's complement<sup>[<a id="anchor-2-src" href="#anchor-2-dst">1</a>]</sup>
form in which the most significant bit is the sign bit.

<br>

For a positive integer, the sign bit is 0 and an arithmetic right-shift
"preserves" this sign bit. The behaviour ends up being the same as logical
right-shift — emptied bits are filled in with 0.
```golang
...
For a negative integer, the sign bit is 1 and an arithmetic right-shift
"preserves" it — emptied bits are filled in with 1.
```golang
...
behaviour ends up being the same.
...
n <<= 1        // 0b00000100 (n = 4)
```
In both cases, the sign bit is lost after the second left-shift. The values
